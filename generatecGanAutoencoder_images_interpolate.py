...
gan.load_weights("weights\\gan32step_7000.h5")

INPUT_DIR="imdb_resized"
labels = np.ones(INTERPOLATION_LENGTH*BATCH_SIZE-INTERPOLATION_LENGTH)
...
for i in range(INTERPOLATION_LENGTH*BATCH_SIZE-INTERPOLATION_LENGTH):
    data.append(latent_vectors[(i//INTERPOLATION_LENGTH)]*(INTERPOLATION_LENGTH-1-i%INTERPOLATION_LENGTH)/(INTERPOLATION_LENGTH-1)+latent_vectors[1+i//INTERPOLATION_LENGTH]*((i%INTERPOLATION_LENGTH)/(INTERPOLATION_LENGTH-1)))
data = np.array(data)
generated_images2=gen.predict(data)
labels = np.ones(INTERPOLATION_LENGTH*BATCH_SIZE-INTERPOLATION_LENGTH)
labels=to_categorical(labels,6)
...
for i in range(INTERPOLATION_LENGTH*BATCH_SIZE-INTERPOLATION_LENGTH):
    data.append(latent_vectors[(i//INTERPOLATION_LENGTH)]*(INTERPOLATION_LENGTH-1-i%INTERPOLATION_LENGTH)/(INTERPOLATION_LENGTH-1)+latent_vectors[1+i//INTERPOLATION_LENGTH]*((i%INTERPOLATION_LENGTH)/(INTERPOLATION_LENGTH-1)))
data = np.array(data)
generated_images3=gen.predict(data)
labels = np.ones(INTERPOLATION_LENGTH*BATCH_SIZE-INTERPOLATION_LENGTH)
labels=to_categorical(labels,6)
...
data = []
for i in range(INTERPOLATION_LENGTH*BATCH_SIZE-INTERPOLATION_LENGTH):
    data.append(latent_vectors[(i//INTERPOLATION_LENGTH)]*(INTERPOLATION_LENGTH-i%INTERPOLATION_LENGTH)/(INTERPOLATION_LENGTH)+latent_vectors[1+i//INTERPOLATION_LENGTH]*((i%INTERPOLATION_LENGTH)/(INTERPOLATION_LENGTH)))
data = np.array(data)
generated_images4=gen.predict(data)
...
for i in range(len(generated_images)):
    ...
    ih1 = cv2.hconcat([generated_images[i],generated_images2[i]])
    ih2 = cv2.hconcat([generated_images3[i],generated_images4[i]])
    i = cv2.vconcat([ih1,ih2])
    i=(i+1)/2.
